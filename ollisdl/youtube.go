...
func YoutubeDownload (sVideoID string, iMaxQuality int, bMP3 bool, sTargetDir string) error {
    ...
    if bMP3 {
        sTempFileWAV := sTempFile + ".wav";
        //fmt.Println("converting to wav");
        oCommandWAV := exec.Command("avconv", "-i", sTempFile, "-vn", sTempFileWAV);
        //oCommand := exec.Command("avconv", "-i", sTempFile, "-vn", "-qscale", "1", sTempFileWAV);
        ...
        oCommandMP3 := exec.Command("lame", "-b", "320", "--cbr", sTempFileWAV, sTempFileMP3);
        errMP3 := oCommandMP3.Run();
        if errMP3 != nil {
            return errMP3;
        }
        os.Remove(sTempFile);
        os.Remove(sTempFileWAV);
